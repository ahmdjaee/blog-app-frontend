    ...
    <>
      ...
      <Spin spinning={isListLoading || isListFetching || isDeleteLoading}>
        {md ? (
          ...
          <>
            {list?.data?.length === 0 || isListError ? (
              <Empty
...
function PostCardPanel({ post = {}, handleDelete }) {
  const navigate = useNavigate();

  return (
    <div className="cs-card-wrapper" style={{ marginBottom: 8 }}>
      <Flex justify="end" align="center">
        <Text
          type={post.published ? "success" : "danger"}
          style={{ marginRight: "auto" }}
        >
          {post.published ? "Published" : "Draft"}
        </Text>
        <Link to={`/user/posts/edit`} state={post}>
          <Button color="primary" size="small" variant="text">
            <EditOutlined />
          </Button>
        </Link>
      ...
      <Flex style={{ cursor: "pointer" }} gap={16}>
        <Image
          width={100}
          height={100}
          style={{
